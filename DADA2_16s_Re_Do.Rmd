...
```{r}
setwd("/Users/bulbulahmed/Documents/SoyabeanProject/DADA_Re_Do_2021/")
path <- "/Users/bulbulahmed/Documents/SoyabeanProject/MiSeq_soya/16s_Reads/"
list.files(path)
...
class(seqtab)
write.table(seqtab, file="SequenceTable_16s_cals_canna_root.txt", sep="\t")
...
write.table(seqtab2, file="SequenceTable_16s_cals_canna_root.txt", sep="\t")
```
# Removing chimeras -------------------------------------------------------
...
#save results:
write.table(seqtab.nochim, "ASVTable_16s_cals_canna_root.txt", sep="\t")
...
```{r}
# Assign taxonomy ---------------------------------------------------------
taxa <- assignTaxonomy(seqtab.nochim,"/Users/bulbulahmed/Documents/Cannabis_project/Cannabis_Cornell_project/hemp_CALS_16S/Root/filtered/silva_nr_v132_train_set.fa", multithread=TRUE)
...
taxa.sp <- addSpecies(taxa, "/Users/bulbulahmed/Documents/Cannabis_project/Cannabis_Cornell_project/hemp_CALS_16S/Root/filtered/silva_species_assignment_v138.fa")
...
taxa.sp.rv <- addSpecies(taxa,"/Users/bulbulahmed/Documents/Cannabis_project/Cannabis_Cornell_project/hemp_CALS_16S/Root/filtered/silva_species_assignment_v138.fa", tryRC=TRUE)
...
asv.16S_cals_canna_root<-seqtab.nochim[,keep_vector==T]  #rows: samples; column: ASV
taxo.16S_cals_canna_root<-taxa.sp[keep_vector==T,]       #rows: ASV; column: phylogenetic levels
```

# Calculation of number of reads and ASV left after pipeline ----------------------
```{r}
##Calculations of reads per samples:
(asv.sum<-rowSums(asv.16S_cals_canna_root))  ##Looks good
min(asv.sum);max(asv.sum)  ##1749 and 30 771
(asv.sum.mean = mean(asv.sum))  ##14 354
(asv.sum.sd = sd(asv.sum))  #6670

(asv.sort<-sort(rowSums(asv.16S_cals_canna_root)))
##Lowest reads: JNR21, JLR42...lowest reads seems to be june_roots, july_roots, june_soil and july_soil

dim(asv.16S_cals_canna_root)
#192 25 854   #### 192 25 852
dim (taxo.16S_cals_canna_root)
#25854     7 ### 25852     7
...
##ASV table
write.table(asv.16S_cals_canna_root, "ASV_16S_cals_canna_root_final.txt", sep="\t")

##Taxonomy table
write.table(taxo.16S_cals_canna_root, "taxo_16S_cals_canna_root_final.txt", sep="\t")


# How to simplified ASV tables for stats ----------------------------------
##Simplified ASV table
asv.16S_cals_canna_root.simplified <- asv.16S_cals_canna_root
colnames(asv.16S_cals_canna_root.simplified) = paste("ASV",c(1:ncol(asv.16S_cals_canna_root)),sep = "")
write.table(asv.16S_cals_canna_root.simplified, "ASV_16S_cals_canna_root_final_simplified.txt", sep="\t")

##Simplified taxonomy table
taxo.16S_cals_canna_root.simplified <- cbind(taxo.16S_cals_canna_root,rownames(taxo.16S_cals_canna_root))  ##He puts the sequences as an extra column (I guess if we want to blast it)
rownames(taxo.16S_cals_canna_root.simplified) <- paste("ASV",c(1:nrow(taxo.16S_cals_canna_root)),sep = "")
write.table(taxo.16S_cals_canna_root.simplified,"taxonomy.cals_canna_root_16S_final_simplified.txt", sep="\t")
```
```{r}
##Importing database
ASV<-read.table("ASV_16S_cals_canna_root_final.txt", header=T, row.names=1, stringsAsFactors=F, skip=1)  ###I kept the original file with the ASV name being the sequence and R is unable to open it, so I am going to import it without the names of the col and then give it back
...
mat=matrix(length(taxo.16S_cals_canna_root[,1]),4,7)
...
# specify rownames 
for (c in 1:7){
  for (i in 1:length(taxo.16S_cals_canna_root[,1])){
    if (is.na(taxo.16S_cals_canna_root[i,c]))
      mat[,c] <- mat[,c] - 1
    as.numeric(mat[,c])
  }
}
mat[2,] <- round(((mat[1,]/length(taxo.16S_cals_canna_root[,1]))*100),1)
mat[3,1]=0
mat[4,1]=0
for (i in 2:7){
